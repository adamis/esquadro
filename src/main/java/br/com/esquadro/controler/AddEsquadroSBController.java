...
public class AddEsquadroSBController implements Runnable {
	...
	public void run() {
		...
		try {
			...
			//HIBERNATE.CFG.XML
			Utils.doCopyFile(new File("EsquadroSB"+ BARRA + "hibernate" + BARRA +"hibernate.cfg.xml"), this.urlProject + BARRA + "src"+ BARRA +"main"+ BARRA +"resources"+ BARRA +"hibernate"+ BARRA +"hibernate.cfg.xml");
			configurarHibernate(new File(this.urlProject + BARRA + "src"+ BARRA +"main"+ BARRA +"resources"+ BARRA +"hibernate"+ BARRA +"hibernate.cfg.xml"), bancoDados);

			//HIBERNATE.PROPERTIES
			Utils.doCopyFile(new File("EsquadroSB"+ BARRA + "hibernate" + BARRA +"hibernate.properties"), this.urlProject + BARRA + "src"+ BARRA +"main"+ BARRA +"resources"+ BARRA +"hibernate"+ BARRA +"hibernate.properties");

			//HIBERNATE.REVENG.XML
			Utils.doCopyFile(new File("EsquadroSB"+ BARRA + "hibernate" + BARRA +"hibernate.reveng.xml"), this.urlProject + BARRA + "src"+ BARRA +"main"+ BARRA +"resources"+ BARRA +"hibernate"+ BARRA +"hibernate.reveng.xml");
			configurarHibernate(new File(this.urlProject + BARRA + "src"+ BARRA +"main"+ BARRA +"resources"+ BARRA +"hibernate"+ BARRA +"hibernate.reveng.xml"), bancoDados);
	...
	private void configurarConexao(BancoDados bancoDados) throws IOException {
		...
		if(bancoDados.getTipo().equalsIgnoreCase("MYSQL")) {
			...
			listProd.add("spring.datasource.url= jdbc:mysql://" + bancoDados.getIp() + ":" + bancoDados.getPorta() + "/" + bancoDados.getNameBd()+"?createDatabaseIfNotExist=true&serverTimezone=UTC");		
		...
		if(bancoDados.getTipo().equalsIgnoreCase("MYSQL")) {
			...
			listDev.add("spring.datasource.url= jdbc:mysql://" + bancoDados.getIp() + ":" + bancoDados.getPorta() + "/" + bancoDados.getNameBd()+"?createDatabaseIfNotExist=true&serverTimezone=UTC");
	...
	//TODO
	private void configurarHibernate(File file, BancoDados bancoDados) throws IOException {
		...
		for (int i = 0; i < readFile.size(); i++) {
			...
			if(bancoDados.getTipo().equalsIgnoreCase("MYSQL")) {//MYSQL
				...
				line = line.replace( "{URL}", "jdbc:mysql://"+bancoDados.getIp()+":"+bancoDados.getPorta()+"/"+bancoDados.getNameBd() );
